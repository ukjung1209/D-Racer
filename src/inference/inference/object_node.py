...
class ObjectNode(Node):
    ...
    def __init__(self):
        ...
        self.session, self.input_name, self.input_size = self._load_model(model_file)
    ...
    def _init_aruco(self):
        ...
        cv2.aruco는 opencv-contrib에 들어 있다. 보드에 없으면
        `pip install opencv-contrib-python` 필요. 4.7+는 ArucoDetector 클래스,
        그 이전은 detectMarkers 함수형 API라 둘 다 대응한다.
        """
        ...
        if not hasattr(cv2, 'aruco'):
            self.get_logger().error(
                'enable_aruco=True인데 cv2.aruco가 없다. '
                'pip install opencv-contrib-python 후 다시 실행하라. 아루코 검출 비활성화.')
    ...
    def image_callback(self, msg: CompressedImage):
        ...
        if self.enable_aruco:
            detections.extend(self._detect_aruco(frame, frame_w, frame_h))
        ...
        if self.publish_debug:
            self._publish_debug(frame, detections, msg)
